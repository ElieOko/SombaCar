...
class CarListingController(
    ...
    @PatchMapping("${CarListingScope.PROTECTED}/{id}/coordinates", produces = [MediaType.APPLICATION_JSON_VALUE])
        ...
        try {
            ...
            sentry.callToMetric(
                MetricModel(
                    ...
                    status = "200",
                    route = "${request.method} /${request.requestURI}",
                    countName = "api.carlisting.coordinates.count",
                    distributionName = "api.carlisting.coordinates.latency",
                )
            )
        }
    }
